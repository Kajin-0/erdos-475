#!/usr/bin/env python3
"""
Generate a route-aware v3 certificate table for the pure worse-only branch.

v2 records hidden-support extraction, target obstruction coverage, and equality
tie-break status.  v3 adds routing coverage for the zero-sum branches using:

    logs/summary_route_bq_bqy_obstructions_p17.json
    logs/summary_route_bq_bqy_obstructions_p23.json

The resulting table is the compact proof-facing branch status:

    zero-sum branches -> target obstruction -> existing route succeeds
    equality branches -> neutral tie-break obligation
"""
...
def route_coverage(fam: str, count: int, route: dict[str, Any]) -> str:
    ...
    obj = ((route.get("route_success_by_family", {}) or {}).get(fam, {}) or {})
    got = int(obj.get("yes", 0) or 0)
    return f"{got}/{count}"
...
def build_rows(p: str, extract: dict[str, Any], bridge: dict[str, Any], genuine: dict[str, Any], route: dict[str, Any], bqy_gap_p23: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for fam in FAMILIES:
        ...
        if count == 0:
            continue
        target_cov = target_coverage(p, fam, count, genuine, bqy_gap_p23)
...
def main() -> int:
    ...
    summary = {
        "rows": rows,
        "interpretation": "zero_sum rows are target-classified and route through existing branch machinery; equality rows remain tie-break obligations",
    }
